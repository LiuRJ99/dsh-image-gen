...
export const InspirationView: FC<InspirationViewProps> = ({ locale, defaultEngine = 'gpt', busy = false, onUsePrompt }) => {
  ...
  return (
    <section className="dsh-ig-inspiration-page" aria-label={t('title')} title={t('subtitle')}>
      <div className="dsh-ig-inspiration-toolbar">
        ...
        <label><span>{t('category')}</span><select aria-label={t('category')} value={category} onChange={(event) => setCategory(event.target.value)}><option value="">{t('all')}</option>{(catalog?.categories ?? []).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
        <label><span>{t('style')}</span><select aria-label={t('style')} value={style} onChange={(event) => setStyle(event.target.value)}><option value="">{t('all')}</option>{(catalog?.styles ?? []).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
        <label><span>{t('scene')}</span><select aria-label={t('scene')} value={scene} onChange={(event) => setScene(event.target.value)}><option value="">{t('all')}</option>{(catalog?.scenes ?? []).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
