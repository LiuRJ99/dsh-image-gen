...
- 将「图像生成」设置移到 DSH 设置的一级侧边栏，打开页面时直接展示配置表单。
...
### Fixed
- 修复灵感库卡片在窄/矮容器（如底部面板只有 185px 高）中被压成 2px 细线的问题：
  - 灵感卡片根节点由 `<button>` 改为 `role="button"` 的 `<div>`，绕开 Chromium 在按钮元素上对 `flex-direction: column` + `aspect-ratio` 子项固有高度计算归零的缺陷；同时消除按钮内嵌按钮的非法结构。
  - 网格声明 `grid-auto-rows: max-content`，卡片声明 `min-height: fit-content`、媒体区声明 `aspect-ratio: 1/1; min-height: 120px`，三重保证行高不再坍塌。
- 修复卡片图片永不加载：网格曾被压到 2px 且位于可视区之外，`IntersectionObserver` 永不触发，图片永远停留在占位符。
- 精简灵感视图占用的垂直空间：移除与标签栏重复的「灵感素材」标题区（约 100px），清理缓存/刷新素材按钮并入工具栏；筛选下拉与工具按钮单行排布（约 46px）。
- 切换「灵感」标签时隐藏画廊专属的图片计数、引擎筛选与网格/列表/表格视图切换，避免无意义控件挤占内容区。
